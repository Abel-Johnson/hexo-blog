...
> 提炼出来要点就是:  
> 由于克隆的时候， themes文件夹较大， 不想git存， 所以加到了`.gitignore`里， 在新的电脑克隆下来这个hexo项目后， 直接启动会报错， 错误提示类似 no layout index.html之类的， 
> 所以解决办法就是把配置文件里的主题名称找到， 克隆主题文件到项目根目录的themes文件夹里， 然后安装两个插件
> 
> ```bash
